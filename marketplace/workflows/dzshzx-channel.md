...
- Read-only architecture review exception: if the work is a repository analysis/report pass that does not modify business code, task state, or shared project config, and the only write is a local HTML report under `.scratch/reports/architecture-review-<timestamp>.html`, task creation may be skipped and the review can proceed inline.
- Complex task: ask whether you may create a Trellis task and enter planning. If the request exceeds the architecture-review exception and the user says no, do not do broad inline implementation.
...
Read-only architecture review exception: if the work is repository analysis only, does not modify business code, task state, or shared config, and only writes a local HTML report under `.scratch/reports/architecture-review-<timestamp>.html`, task creation may be skipped.
Complex task outside that exception: ask the user if you can create a Trellis task and enter the planning phase. If the user says no, explain, clarify scope, or suggest a smaller split.
[/workflow-state:no_task]
